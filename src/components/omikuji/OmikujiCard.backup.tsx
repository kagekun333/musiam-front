...
export default function OmikujiCard({
  ...
  const subHeader: string | undefined = undefined; // ENのサブタイトルは非表示
  ...
  return (
    <article
      ...
      style={{
        ...
        ["--pattern-color" as any]: theme.isDark ? "#ddd" : "#777",
        // 麻の葉はライト濃いめ / ダーク控えめ（globals.css の var(--pattern-opacity) を上書き）
        ["--pattern-opacity" as any]: theme.isDark ? "0.20" : "0.24",
        // ["--edgeY" as any]: "6%",   // 縁の太さを変えたくなったら解放
        // ["--edgeX" as any]: "2.5%",
      }}
    >
      {/* 背景（和紙グラデ） */}
      ...
      {/* 外枠アクセント：確実に色が出るよう ring ではなく border を使用 */}
      <div
      ...
      <div className="relative z-10 mx-auto flex h-full w-[min(92%,880px)] flex-col p-8">
        ...
        <header className="mb-4">
          ...
          {subHeader && <p className="mt-1 text-sm opacity-70">{subHeader}</p>}
