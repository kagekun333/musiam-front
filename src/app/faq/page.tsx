...
const FAQS: QA[] = [
  // 伯爵MUSIAMについて
  {
    cat: "伯爵MUSIAMについて",
    q: "伯爵MUSIAMとは何ですか？",
    a: "ABI伯爵が館主を務める、音楽と書物でできた一つの「国（館）」です。350を超える作品（楽曲216リリース・書籍134）を、たった一人の作り手と複数のAIだけで制作・公開しています。作品はバラ売りの新譜ではなく、地続きに繋がった一つの世界観として展示されています。",
  },
  {
    cat: "伯爵MUSIAMについて",
    q: "ABI伯爵とは誰ですか？",
  ...
  },
  {
    cat: "伯爵MUSIAMについて",
    q: "どれくらいの数の作品がありますか？",
    a: "公開作品は合計350以上です。内訳はオリジナル楽曲が216リリース（アルバムに複数曲収録しているものもあり、収録曲そのものは460曲以上）、出版書籍が134。ジャンルはダンス・ハウス・トランス・ジャズ・レゲエからニューエイジ・ヒップホップまで多岐にわたり、書籍も叙事詩的ファンタジーから現代ビジネス風刺、SFまで揃っています。",
  },
  // AI制作について
  {
    cat: "AI制作について",
    q: "本当にAIだけで作っているのですか？",
    a: "いいえ、AIに一任しているわけではありません。曲や物語の着想はまず伯爵の中に生まれ、それをAIと幾度も対話しながら作品へと形にしていきます。そうして生まれた候補の中から最高の一つを選び抜き、マスタリングや編集も人の手で仕上げる——着想から仕上げまで、AIと伯爵は五分五分のパートナーです。",
  },
  {
    cat: "AI制作について",
    q: "AIで作ると品質は落ちませんか？",
    a: "量を作れること自体は品質を下げません。品質を決めるのは、AIとの対話から生まれた無数の候補から何を残し何を捨てるかという人間の選別眼です。伯爵MUSIAMでは百の候補から一つを選び抜くほどの選別を行い、世界観から外れた作品は公開しません。",
  },
  {
    cat: "AI制作について",
    q: "同じ作り方を自分でも学べますか？",
...
function renderAnswer(text: string) {
  ...
  for (let i = 0; i < parts.length; i++) {
    if (i % 2 === 1) {
      nodes.push(
        <Link key={i} href={parts[i]} style={{ color: "var(--rnv-text-amber)" }}>
          （{parts[i]}）
        </Link>
      );
    } else if (parts[i]) {
  ...
  return (
    <>
      ...
      >
        ...
        >
          解決しないご質問は{" "}
          <Link href="/business" style={{ color: "var(--rnv-text-amber)" }}>
            法人の門
          </Link>{" "}
          の連絡先から、あるいは{" "}
          <Link href="/letters" style={{ color: "var(--rnv-text-amber)" }}>
            伯爵の手紙
          </Link>{" "}
          をお読みください。
        </p>
