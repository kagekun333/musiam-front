...
type RankEn = typeof RANK_ORDER[number];
...
function withUtm(href: string, rank: string) {
  const sep = href.includes("?") ? "&" : "?";
  return `${href}${sep}utm_source=omikuji&utm_medium=reco&utm_campaign=${encodeURIComponent(rank)}`;
}

/** シード乱数（今日+ランクで“日替わり固定”のランダム推薦） */
...
function seededSampleArray<T>(arr: T[], n: number, seedStr: string): T[] {
  if (n <= 0) return [];
...
export default function OmikujiPage() {
  ...
  const draw = () => {
    ...
    // 凶以外：作品から“日替わり固定ランダム”で抽出（6件）
    if (works && works.length > 0 && pick.rank_en.toLowerCase() !== "bad luck") {
      const seedKey = `${todayYMD()}::${pick.rank_en}`;
      const seeded = seededSampleArray(works, 6, seedKey);
      setPickedWorks(seeded.length ? seeded : sampleArray(works, 6));
    } else {
      setPickedWorks(null);
    }
  };
          ...
          {recommendation && recommendation.kind === "fixed" && (
            <section className="mt-6 rounded-xl border bg-white/70 p-5" style={{ borderColor: `${accent}66` }}>
              ...
              {recommendation.href && (
                <div className="mt-3">
                  {(() => {
                    const dest = withUtm(recommendation.href!, entry.rank_en);
                    return isExternalUrl(dest) ? (
                      <a
                        href={dest}
                        target="_blank"
                  ...
                  <div key={w.id} className="group rounded-lg border border-zinc-200 overflow-hidden bg-white/80 hover:shadow-sm transition">
                    ...
                    <div className="p-2">
                      ...
                      {(w.href || w.previewUrl) && (
                        <div className="mt-2">
                          {(() => {
                            const dest = withUtm(w.href || w.previewUrl!, entry.rank_en);
                            return isExternalUrl(dest) ? (
                              <a
                                href={dest}
                                target="_blank"
