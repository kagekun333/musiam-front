...
type Props = {
  ...
  bundle: OmikujiEntry[];
  imageBase: string;
  poemOriginal: Record<number, string[]>;   // JAの四句マップ
  poemEn: Record<number, string[]>;         // ENの四句マップ
};
...
const OracleLangPage: NextPage<Props> = ({ lang, bundle, imageBase, poemOriginal, poemEn }) => {
  const t = UI[lang];
  ...
  const poemEnLines = rec ? (poemEn[rec.id] ?? []) : [];
...
export const getStaticProps: GetStaticProps<Props> = async (ctx) => {
  ...
  // EN: 対訳用マップ（追加）
  const enMod = await import(`../../../dist/omikuji_bundle.en.json`);
  const enBundle = (enMod.default ?? enMod) as OmikujiEntry[];
  const poemEn: Record<number, string[]> = {};
  for (const e of enBundle) poemEn[e.id] = e.poem?.hanbun ?? [];

  const imageBase = process.env.NEXT_PUBLIC_OMIKUJI_IMAGE_BASE || "/images/kannon100";
  return { props: { lang, bundle, imageBase, poemOriginal, poemEn } };
};
