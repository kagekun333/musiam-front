...
export default function OmikujiPage() {
    ...
    async function loadWorks() {
      ...
      for (const p of candidates) {
        try {
          ...
          const list: WorkItem[] = raw.map((w: any) => ({
            id: String(w.id ?? w.slug ?? w.title ?? Math.random()),
            title: String(w.title ?? w.titleJa ?? w.titleEn ?? "Untitled"),
            type: w.type ?? w.kind ?? "",
            href: w.href ?? w.url ?? w.link ?? undefined,
            cover: w.cover
  ...
  const draw = () => {
    ...
    if (works && works.length > 0 && pick.rank_en.toLowerCase() !== "bad luck") {
      setPickedWorks(sampleArray(works, 6));
    } else {
      setPickedWorks(null);
    }
  };
        ...
        <>
          ...
          {drawsToday >= 2 && (
            <div className="mt-5 rounded-lg border border-zinc-300 bg-white/70 p-4 text-sm">
              {lang === "ja" ? (
                <>おみくじは最初の一枚こそが神の導きであり、何度も引くものではありません。<br />明日またお試しください。</>
              ) : (
                <>The first draw is the true guidance; omikuji isn’t meant to be drawn repeatedly.<br />Please try again tomorrow.</>
              )}
            </div>
          )}
          ...
          {recommendation && recommendation.kind === "fixed" && (
            <section className="mt-6 rounded-xl border bg-white/70 p-5" style={{ borderColor: `${accent}66` }}>
              ...
              <div className="mb-2 text-[15px] font-medium">
                {lang === "ja" ? "星海の眠り — ヒーリング音楽" : "Starsea Slumber — Healing Ambient"}
              </div>
              <div className="text-sm opacity-70">
                {lang === "ja" ? "今日はゆっくり休んでください。" : "Please take it slow and rest today."}
              </div>
              <div className="mt-3">
                <Link href="https://open.spotify.com/album/79CuhhEhb0GtBzgkk7fwsY" className="inline-flex items-center gap-2 rounded-md border border-zinc-300 px-3 py-1.5 text-sm hover:bg-zinc-50">
          ...
          {recommendation && recommendation.kind === "works" && (
            <section className="mt-6 rounded-xl border bg-white/70 p-5" style={{ borderColor: `${accent}66` }}>
              ...
              <div className="text-sm opacity-70 mb-3">
                {lang === "ja" ? "今日の御籤に寄り添う作品たち。気になったものからどうぞ。" : "A few works that vibe with today’s draw. Pick what intrigues you."}
              </div>
